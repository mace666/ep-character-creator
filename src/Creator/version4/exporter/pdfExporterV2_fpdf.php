			...
			//EGO ================================================================ 
				...
				foreach($skillList as $skill)
				{
					if($skill->baseValue > 0 || $skill->defaultable == EPSkill::$DEFAULTABLE)
					{
						...
						if(!empty($skill->specialization))
						{
							...
							$pdf->Text($apt_x,($apt_y-$y_space+2),formatIt("spec[".$skill->specialization."]"));//Skill speci
							$hadSpec = true;
							if($i%2 == 0) $pdf->SetFont('Lato-Reg','',$fontsize);
							else $pdf->SetFont('Lato-Lig','',$fontsize);
						}
				...
				//END EGO PAGE
					...
					foreach($morphs as $morph){
						...
						$pdf->Text( 50, 19,formatIt($morph->age));//morph apprent age
						...
						foreach($stats as $s)
						{
							if($i%2 == 0)$pdf->SetFont('Lato-Reg','',8);
							else $pdf->SetFont('Lato-Lig','',8);
						...
						foreach($skillList as $skill)
						{
							if($skill->baseValue > 0 || $skill->defaultable == EPSkill::$DEFAULTABLE)
							{
								...
								$pdf->Text( ($apt_x+53), $apt_y, formatIt($skill->getValue()));//Skill total value;
								if($hadSpec) $apt_y += ($y_space+8);
								else $apt_y += $y_space;
								$i++;
							}
